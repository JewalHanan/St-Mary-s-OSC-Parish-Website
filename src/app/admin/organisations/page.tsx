...
export default function OrganisationsManager() {
    ...
    const addOrg = () => {
        if (!newOrgName.trim()) return alert('Organisation name is required.');
    ...
    const deleteOrg = (id: number) => {
        if (confirm('Delete this organisation and all its bearers?')) {
    ...
    return (
        <div className={styles.managerContainer}>
            <header className={styles.header}>
                <div>
                    <h1>Organisations</h1>
                    <p>Manage church organisations and their office bearers. ({orgs.length} organisations)</p>
                </div>
                <Button variant="primary" onClick={() => setShowAddOrg(true)}>➕ Add Organisation</Button>
            </header>

            {showAddOrg && (
                <Card style={{ marginBottom: '1.5rem', padding: '1.5rem', background: 'var(--card-bg)' }}>
                    <h3 style={{ color: 'var(--color-gold)', marginBottom: '1rem' }}>New Organisation</h3>
                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <input value={newOrgName} onChange={e => setNewOrgName(e.target.value)} placeholder="Organisation Name (e.g. OCYM)" style={{ ...inputStyle, flex: 1, minWidth: 200 }} />
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', marginTop: '0.5rem' }}>Click the logo square to upload a symbol/icon for this organisation.</p>
                </Card>
            )}
            ...
            {orgs.map(org => (
                <Card key={org.id} style={{ marginBottom: '1.5rem', padding: '1.5rem', background: 'var(--card-bg)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        <div style={{ cursor: 'pointer', flex: 1, display: 'flex', alignItems: 'center', gap: '1rem' }} onClick={() => setExpandedOrgId(expandedOrgId === org.id ? null : org.id)}>
                            {/* Org logo or initial */}
                            {org.logo
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1rem' }}>
                                    {org.bearers.map(bearer => (
                                        <Card key={bearer.id} style={{ padding: '1.25rem', textAlign: 'center', background: 'var(--input-bg)' }}>
                                            {bearer.image ? (
                                                <img
                                                    src={bearer.image} alt={bearer.name}
                                                    style={{ width: '80px', height: '80px', borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--accent-primary)', margin: '0 auto 1rem' }}
                                                />
                                            ) : (
                                                <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'var(--card-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', margin: '0 auto 1rem', border: '3px solid var(--card-border)' }}>
                                                    👤
                                                </div>
                                            )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                ...
                                <div>
                                    ...
                                    {bearerForm.image && (
                                        <Button variant="secondary" onClick={() => setBearerForm({ ...bearerForm, image: '' })} style={{ padding: '6px 12px', fontSize: '0.85rem', marginLeft: '8px' }}>
                                            Remove
                                        </Button>
                                    )}
                                </div>
