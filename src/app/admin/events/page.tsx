...
export default function EventsManager() {
    ...
    const openAdd = () => {
        if (events.length >= MAX_EVENTS) return alert(`Maximum of ${MAX_EVENTS} events allowed. Delete one first.`);
        setEditingId(0);
    ...
    const handleIconUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (url) setForm(prev => ({ ...prev, icon: url }));
        else alert('Icon upload failed. Please try again.');
        setUploading(false);
        e.target.value = '';
    };
    ...
    return (
        <div className={styles.managerContainer}>
            <header className={styles.header}>
                ...
                <Button variant="primary" onClick={openAdd} style={{ opacity: events.length >= MAX_EVENTS ? 0.5 : 1 }}>
                    ➕ Add New Event
                </Button>
            </header>
            ...
            <Card className={styles.tableCard}>
                <div className={styles.tableResponsive}>
                    <table className={styles.dataTable}>
                        <thead><tr><th>Color</th><th>Event Title</th><th>Date</th><th>Time</th><th>Type</th><th>Actions</th></tr></thead>
                        <tbody>
                            {events.map(event => (
                                <tr key={event.id}>
                                    <td>{event.icon ? <img src={event.icon} alt="icon" style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover' }} /> : <span style={{ width: '14px', height: '14px', borderRadius: '50%', backgroundColor: event.color, display: 'inline-block' }} />}</td>
                                    <td style={{ fontWeight: 'bold' }}>{event.title}</td>
                                    <td>{event.date}</td>
                                    <td>{event.time}</td>
                                    <td style={{ textTransform: 'uppercase', fontSize: '0.8rem' }}>{event.type}</td>
                                    <td><div style={{ display: 'flex', gap: '8px' }}><Button variant="outline" onClick={() => openEdit(event)} style={{ padding: '6px 12px', fontSize: '0.8rem' }}>Edit</Button><Button variant="secondary" onClick={() => deleteEvent(event.id)} style={{ padding: '6px 12px', fontSize: '0.8rem' }}>Delete</Button></div></td>
                                </tr>
                            ))}
                            {events.length === 0 && (<tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>No events scheduled.</td></tr>)}
                        </tbody>
                    </table>
                </div>
            </Card>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
                    <Card style={{ width: '100%', maxWidth: '500px', background: 'var(--color-navy)', padding: '2rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Event Description (optional)" rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
                            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                ...
                                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} style={{ ...inputStyle, flex: 1 }}>
                                    <option value="feast">Feast</option><option value="meeting">Meeting</option><option value="special">Special</option><option value="other">Other</option>
                                </select>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                {uploading ? <span style={{ fontSize: '1.5rem' }}>⏳</span> : form.icon ? <img src={form.icon} alt="icon preview" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--accent-primary)' }} /> : <span style={{ fontSize: '2rem' }}>🎪</span>}
                                <button type="button" onClick={() => iconInputRef.current?.click()} disabled={uploading} style={{ ...inputStyle, width: 'auto', cursor: 'pointer', padding: '8px 14px' }}>
                                    {uploading ? '⏳ Uploading…' : `📷 ${form.icon ? 'Change Icon' : 'Upload Icon (optional)'}`}
                                </button>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
                            ...
                            <Button variant="primary" onClick={save} disabled={uploading}>{uploading ? '⏳ Uploading…' : 'Save'}</Button>
                        </div>
                    </Card>
                </div>
            )}
